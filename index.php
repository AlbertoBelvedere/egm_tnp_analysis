...
if ($_GET['noplots']) {
    ...
    $filenames = glob("{*png,*.jpeg,*jpg}",GLOB_BRACE); sort($filenames);
...
<h2><a name="files">Tex files</a></h2>
<ul>
<?
$filenames = glob("{*tex}",GLOB_BRACE); sort($filenames);
foreach ($filenames as $filename) {
    if (file_exists($filename)) {
        print "<li><a href=\"$filename\">$filename</a></li>";
        print "<pre class='readme'>\n"; readfile($filename); print "</pre>";
    }
}
?>
</ul>
