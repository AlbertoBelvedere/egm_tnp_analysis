...
def histFitterNominal( sample, tnpBin, tnpWorkspaceParam ):
    ...
    hP = infile.Get('%s_Pair' % tnpBin['name'] )
    fitter = tnpFitter_MVA( hP, tnpBin['name'] )
    infile.Close()
    ...
    histZLineShapeP = fileTruth.Get('%s_Pair'%tnpBin['name'])

    fitter.setZLineShapes(histZLineShapeP)
